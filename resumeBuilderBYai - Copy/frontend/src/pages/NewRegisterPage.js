// frontend/src/pages/NewRegisterPage.js
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { register } from '../services/newAuthService';
...
function NewRegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setError('Failed to register. Please try another username or email.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center">
      <div className="p-6 border border-slate-800 rounded-2xl bg-slate-900/40 w-full max-w-sm">
        ...
        <p className="text-center mt-4 text-sm">
          ...
          <a href="/new-login" className="text-indigo-400 hover:underline">
            Login here
          </a>
        </p>
      </div>
